...
def check_is_vuln():
	...
	# unpatched version, error because of 'a=a' argument
	r, w = os.pipe()
	pid = os.fork()
	if not pid:
		# child
		os.dup2(w, 2)
		execve(SUDO_PATH, [ b"sudoedit", b"-s", b"a=a", None ], [ None ])
		exit(0)
	# parent
	os.close(w)
	os.waitpid(pid, 0)
	...
	if err.startswith('sudoedit: you may not specify environment'):
		return True
